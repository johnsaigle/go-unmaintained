...
var (
	count  = flag.Int("count", 500, "Number of popular packages to fetch")
...
func main() {
	...
	fmt.Printf("Building popular packages cache...\n")
	fmt.Printf("  Count: %d packages\n", *count)
	fmt.Printf("  Output: %s\n", *output)
	fmt.Printf("  Inactive threshold: %d days\n", *maxAge)
	fmt.Println()

	entries, err := buildCache(*token, *count, *maxAge)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error building cache: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("Fetched %d packages\n", len(entries))
